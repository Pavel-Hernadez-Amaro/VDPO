...
B2XZG_1d_2_var <- function(B, pord = c(2, 2), c = c(10, 10)) {
  # without intercept
  ...
  # T=cbind(T_n[,-c(1,pord[1]+1)],T_s)

  # all.equal(t(T)%*%T,diag(c1+c2-2)) # THIS IS TRUE
  # all.equal(T%*%t(T),diag(c1+c2)) # BUT THIS IS NOT
